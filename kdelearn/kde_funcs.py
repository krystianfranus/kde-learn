...
class KdeClassifier:
    ...
    def fit(
        ...
        if prior is None:
            self.prior = np.zeros(self.ulabels.shape)
            for idx, label in enumerate(self.ulabels):
                mask = labels_train == label
                self.prior[idx] = labels_train[mask].shape[0] / x_train.shape[0]

        self.bandwidth = scotts_rule(x_train) if shared_bandwidth else None
        return self

    def _compute(self, x_test: ndarray):
        scores = np.zeros((x_test.shape[0], self.ulabels.shape[0]))
        for idx, label in enumerate(self.ulabels):
            mask = self.labels_train == label
            kde = Kde(self.kernel_name).fit(self.x_train[mask], self.bandwidth)
